...
# Subset: Table MS00111.txt and PROBE_CODE "AIRCEN08" (5 minute air temperatures)
# Average 5 minute data to 15 minutes for ODM format
...
#  - create separate data file from MS00111.txt for PROBE_CODE "AIRCEN08.csv"
...
#        filter(PROBE_CODE==probe_code) %>%
#        write_chunkwise(file=file.path(dir_data, paste(probe_code, "csv", sep=".")))
#
#    df_met <- read_csv(file=file.path(dir_data, paste(probe_code, "csv", sep=".")))
#    method_code <- unique(df_met$AIRTEMP_METHOD)
## end section
...
# read information from ODM tables and assign to columns of "DataValues.csv"
    ...
    SiteCode <- odm_table_sites$SiteCode[1]  #"AIRCEN08"
...
# or assign directly
#df_odm$SourceCode <- "AND"
#df_odm$SiteCode <- "AIRCEN08"
#df_odm$SiteCode <- "AIRTEMP_MEAN"

# read original data
    df_met <- read_csv(file=file.path(dir_data, paste(probe_code, "csv", sep=".")))

# determine 15 minute time intervals
# drop the records that didn't parse into datetime
    df_met <- drop_na(df_met, DATE_TIME)
# 15 minute intervals and add column to tibble df_met
